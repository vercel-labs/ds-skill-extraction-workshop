...
- **Citation discipline for default values:** a default-value claim cites whichever source actually STATES the default: the d.ts line ONLY when it carries an `@default` JSDoc tag or a literal default in the type; otherwise the implementation line where the default is destructured (`Component.js:<line>` / `ComponentBase.js:<line>`). Citing the d.ts for a default it does not state is an IMPRECISE claim — the adversarial audit treats it as a citation defect even when the value itself is correct. Same discipline at lower severity: re-read the cited line right before writing it into prose — adjacent-line drift (cite `:73`, actual `:72`) is the same defect class. The claim also lands as a `CITE:` row whose snippet is the default-stating text (the destructure or `@default` line, never just the value) — `scripts/verify-citations.sh` enforces this bullet mechanically; see `references/validate.md` (Citation-verification step).
