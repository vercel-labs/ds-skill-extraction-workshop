...
Every component on the confirmed slate (the proposing set the user approved at the Phase 1 gate) must materialize in the produced skill with its own contract section — `references/components/<kebab-name>.md` in per-file mode or a `## <ComponentName>` section in `references/components.md` in single-file mode — carrying props, variants, pitfalls, and every rule its upstream machine-readable docs + prose carry. No two-tier coverage: riding on composition exemplars, worked examples, a sibling component's file, or the `## Other re-exports` tier does not satisfy the rule. See `references/component-extraction.md` (Full-coverage rule).

**Why:** Without the rule, extraction quietly splits the slate into two tiers — components with full contracts and components that only appear inside exemplars or another component's prose. A downstream agent loading the routing table for a second-tier component finds no API surface, no pitfalls, and no NEVER-rules, so it generates from memory — the exact hallucination class the skill exists to prevent. The split is invisible at persist time because every slate name still "appears somewhere" in the skill.

**How to enforce:** Phase 3 declares the slate in the produced SKILL.md under `## Component slate` (per `references/skill-template.md` + `references/persist.md`). `scripts/check-skill-docs.sh` produced-mode check `SLATE_COVERAGE` cross-checks each declared name against the emitted contract sections and hard-fails on any slate component without one; meta-mode check `FULL_COVERAGE_RULE_PRESENT` guards the rule prose and the template bullet against regression. `scripts/tests/run-tests.sh` exercises both branches via the `pass-slate-coverage` / `fail-slate-coverage` fixtures.
...
### shell/fixed-viewport-height

Whenever Phase 2 lifts a shell wrapper that sets a FIXED viewport height — `style={{ height: "100vh" }}` (or `height: 100vh` / `height: 100dvh` in a lifted Companion CSS rule on `html`, `body`, or the base-surface element) — the lift stays verbatim (never "tidy" the snippet), but the produced SKILL.md MUST include a Hard Rule flagging the construct: pages taller than the viewport clip or lose scrolling under a fixed height; consumer apps should use `minHeight` for the fill-the-viewport intent. The Hard Rule cites this slug.

**Why:** Reference projects are often single-screen showcases where `height: 100vh` is harmless, so the foot-gun ships silently inside verbatim wiring. A downstream agent pastes the Setup block, builds a content page taller than the viewport, and the shell clips it — scroll dies inside the fixed-height wrapper. The verbatim-lift contract is correct (do not edit the source's wiring); the missing piece is the prescriptive warning that fires at emit time, exactly like the other shell invariants. Unlike the three slugs above, PRESENCE of the construct is the trap — the others fire on a missing pairing.

**How to apply:** Phase 2 detects the construct while enumerating wiring steps in `.extract-ds-skill-scratch/shell-invariants.md` per `references/validate.md` (Shell-invariant extraction step). Phase 3 materializes a Hard Rule in the produced SKILL.md `## Hard rules` section AND a Layer B Bad/Good/Why row in the produced `references/anti-patterns.md` under the `shell/` namespace. No deterministic script gate — warn-only; the generic `SHELL_INVARIANTS` post-emit check already asserts shell-vocabulary Hard Rules exist when Setup ships wiring.
...
- `component/...` for component-level rules (mostly Layer A) — examples: `component/button-inactive-vs-disabled`, `component/textinput-requires-formcontrol`, `component/button-no-aria-label-with-text`. Layer C `component/...` slugs also exist for meta-skill extraction-completeness discipline that fires per-component (e.g. `component/anti-substitution-dropped`, `component/reexport-tier-invisible`, `component/slate-contract-missing`) — the Layer is named in each slug's subsection body, not in the namespace.
- `asset/...` for asset violations (Layer A or Layer B depending on cite) — examples: `asset/raw-svg-instead-of-icon`, `asset/inlined-logo-instead-of-package-import`, `asset/closed-prop-surface` (icon prop interface narrower than `React.SVGProps`; see `references/component-extraction.md`, Closed-prop-surface check).
...
- `shell/fixed-viewport-height` (shell, when the lifted wiring sets a fixed viewport height on the shell wrapper) — Bad: `<BaseSurface style={{ height: "100vh" }}>` copied as-is into the consumer app; Good: `<BaseSurface style={{ minHeight: "100vh" }}>`; Why: fixed height clips content taller than the viewport and breaks page scrolling; `minHeight` preserves the fill intent.
