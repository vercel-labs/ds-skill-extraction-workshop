...
- Probe-derived tokens (source-blocked recovery fallback ONLY — `references/validate.md`, Compiled-CSS fallback): when the phase-2 handoff carries a `probe-recovered-tokens.txt` manifest, materialize those rows under a dedicated `## Probe-derived tokens [probe-derived]` section at the BOTTOM of the produced tokens file (multi-family mode: a single `references/tokens/probe-derived.md`), never interleaved with source-cited entries. The section preamble states the provenance and the loss in two short sentences: values were recovered from the rendered docs page because the token source was `[private-blocker]`, reflecting one page in one rendered mode; semantic names are lost (the synthetic `--probe-*` rows carry no DS naming authority). Every row keeps the `[probe-derived]` tag in place of a `file:line` cite. `scripts/check-skill-docs.sh` (produced mode, `PROBE_DERIVED_TOKENS`) gates the dedicated-section placement post-emit. Conflict rows the phase-2 handoff marked source-wins are NOT materialized here — the source-cited entry already covers them.
