...
If a public docs URL was accepted in Phase 1 AND the user has not opted out, run the rendered-site probe in addition (opt-out phrase: **"skip rendered probe"** at any point before Phase 2 closes; default for CI and unattended runs is skip). Write the probe manifest to `.extract-ds-skill-scratch/probe-manifest.txt` — one `<token> | <declared-value> | <source-cite file:line> [| <css-selector>]` row per source-extracted color, font-family, and base-spacing token — then run `bash scripts/probe-rendered.sh --url <accepted-docs-url> --manifest .extract-ds-skill-scratch/probe-manifest.txt --out .extract-ds-skill-scratch/probe-report.txt`. The probe renders the docs site headless and diffs each declared value against the computed CSS custom-property value; every `MISMATCH`/`UNRESOLVED` row arrives as a ready-made `[VERIFY: rendered-probe ...]` line — append those lines to the same `[VERIFY]` tally the rest of Phase 2 carries, and append the probe's `PROBE_RESULT=` summary line to the proof point immediately after the `TOKEN_COVERAGE` line. The probe ANNOTATES, never overrides: source extraction stays authoritative, and a mismatch is a finding (theme override, build-time transform, stale source declaration), not a validation failure. Skips are logged verbatim and Phase 2 continues unchanged (`PROBE_SKIPPED=no-docs-url` when no docs URL is in scope; `PROBE_SKIPPED=browsers-unavailable` when browser binaries are missing — they install on the HOST, never inside a sandbox; see the script header). A non-zero probe exit logs `[VERIFY: rendered-probe failed — <PROBE_FAILED line verbatim>]` and Phase 2 continues — the probe never blocks the approval gate. Full manifest and output contract: `references/validate.md` (Rendered-site probe step).

If token-class source extraction (color, font, spacing) returned `[private-blocker]` — the DS ships only compiled CSS, no readable token source — AND a public docs URL was accepted in Phase 1 AND the user has not opted out (same phrase: "skip rendered probe"), run the probe's **recover fallback** instead of stalling: `bash scripts/probe-rendered.sh --recover --url <accepted-docs-url> --out-manifest .extract-ds-skill-scratch/probe-recovered-tokens.txt`. Every recovered row carries the `[probe-derived]` provenance tag in place of a `file:line` cite, and recovered values largely lose semantic names (the `--probe-*` rows are synthetic — you get a computed value, not the DS's token name). Probe-derived tokens are SECOND-CLASS: they never replace a source-cited token, and when both exist for the same value and conflict, source wins and the conflict is logged as one `[VERIFY: probe-recover conflict — ...]` line in the Phase 2 tally, never silenced. Do NOT widen the fallback to DSes whose token source is readable — it exists only for the `[private-blocker]` token case; anything wider breaks the source-first, citation-first contract. Phase 3 materializes recovered tokens under a dedicated `## Probe-derived tokens [probe-derived]` section in the produced tokens file (never interleaved with source-cited entries) with a short provenance-and-loss note, and `check-skill-docs.sh` produced mode surfaces the split as the `PROBE_DERIVED_TOKENS=` tally. Skips degrade exactly like the diff probe (`PROBE_SKIPPED=...` exit 0; `PROBE_FAILED=navigation` exit 1 → log one line and continue): Phase 2 proceeds with the `[private-blocker]` gap recorded as-is, which remains the correct default. Full contract: `references/validate.md` (Compiled-CSS fallback).
...
| Running Phase 2 validation in `.extract-ds-skill-scratch/` | `references/validate.md` | Typecheck + grep-resolves protocol, `[VERIFY]` tally, wait-gate, rendered-site probe contract (opt-in, `scripts/probe-rendered.sh`), incl. the compiled-CSS recover fallback for source-blocked DSes |
...
- After scaffold completes, run `scripts/check-skill-docs.sh`. This is the post-emit consistency check (it includes the produced-mode `CITATION_VERIFICATION` gate — every `file:line` cite in EVERY persisted file re-verified, carried-over files included; see `references/persist.md`, Citation verification); its output IS the audit result. Do not declare the skill saved before this script exits 0.
