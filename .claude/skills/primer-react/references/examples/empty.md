...
- Empty states use the experimental `Blankslate` composed from named subcomponents: `Visual` (an octicon at `size={32}`), `Heading`, `Description`, `PrimaryAction`, `SecondaryAction` — in that order.
- `Blankslate` is imported from `@primer/react/experimental`, NOT the root entrypoint — experimental components live on a separate import path.
- Actions take an `href` directly (`Blankslate.PrimaryAction href="/repos"`); there is no nested Button.
- The blankslate still sits inside a `PageLayout` so an empty surface reads like a routed page, not a floating card; `spacious border` gives it the framed, centered treatment.
