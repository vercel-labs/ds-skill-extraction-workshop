...
- Multi-section page: a `wrap="wrap"` horizontal `Stack` of token-painted stat cards on top, a `<section>` with a `Timeline` below, all inside one vertical `Stack gap="spacious"`.
- Stat cards are the same token recipe as the form card but lighter: `--bgColor-default` + `--borderColor-default` + `--borderRadius-large` + `--shadow-resting-small`, with `flex: "1 1 200px"` so they reflow responsively.
- Each card's header row is a horizontal `Stack justify="space-between"` (muted label left, octicon right), then a large `Heading` for the value, then a muted delta `Text`.
- A section header can pair a `Label variant="accent"` with a `CounterLabel` for a count badge — `CounterLabel` wraps the number, `Label` carries the qualifier text.
- Activity feeds use `Timeline > Timeline.Item > (Timeline.Badge + Timeline.Body)`; the badge's `variant` (`success`/`open`/`closed`/`accent`) is driven from data and wraps an octicon.
- Heading hierarchy: `PageHeader.Title` for the page, `h2` for section, `h3` for card values.
