...
- Sidebar-nav layout uses three `PageLayout` regions: `Header`, `Pane position="start" width="medium"`, and `Content` — the pane is where the nav lives, not a hand-built column.
- `NavList` carries an `aria-label`; each `NavList.Item` takes an `href`, marks the active item with `aria-current={current ? "page" : undefined}`, and puts its octicon in a `NavList.LeadingVisual` slot.
- Content is divided into `<section>`s separated by an `aria-hidden` divider div bordered with `--borderColor-muted` — sections, not cards, for a settings body.
- Each section opens with a tight `Heading as="h2" variant="medium"` + muted `Text` description pair, then a vertical `Stack` of `FormControl` rows.
- Each section's submit is a single right-aligned `Button variant="primary"` in a `Stack justify="end"` — one action per section, not one for the whole page.
- Heading levels track the page outline: `h1` for the page title, `h2` per section.
