...
- A sidebar-nav page uses `PageLayout` with three regions: `PageLayout.Header` (page title), `PageLayout.Pane position="start"` (the nav), `PageLayout.Content` (the form sections).
- The nav is a `NavList` with an `aria-label`; each `NavList.Item` carries an octicon in `NavList.LeadingVisual` and marks the active route with `aria-current="page"` (set to `undefined`, not `false`, when inactive).
- Content is divided into `<section>`s, each a vertical `Stack` opening with a `Heading as="h2" variant="medium"` + a muted `Text` description.
- Section dividers are a hand-painted `<div>` with `borderTop: 1px solid var(--borderColor-muted)` and `aria-hidden` — decorative rules carry the muted border token and are hidden from AT.
- Each section ends with its own action: a horizontal `Stack` with `justify="end"` holding a single `primary` Button.
