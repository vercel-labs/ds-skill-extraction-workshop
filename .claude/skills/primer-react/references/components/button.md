...
- `variant?: 'default' | 'primary' | 'invisible' | 'danger' | 'link'` — visual emphasis; default `'default'`. `node_modules/@primer/react/dist/Button/types.d.ts:4,18`
- `size?: 'small' | 'medium' | 'large'` — default `'medium'`. `dist/Button/types.d.ts:5,22`
...
- `disabled?: boolean` — native disabled. Prefer `inactive` when the goal is "looks inactive". `dist/Button/types.d.ts:28`
- `block?: boolean` — fills container horizontally. `dist/Button/types.d.ts:32`
...
- Native passthrough: `React.ButtonHTMLAttributes<HTMLButtonElement>` (`onClick`, `type`, `name`, …). `dist/Button/types.d.ts:46`
...
- `inactive` is the keyboard-accessible "disabled-look" — the button stays in tab order and can be focused. Reach for `disabled` only when the action truly cannot be performed by anyone right now (form-level invalid state, missing permission). `dist/Button/types.d.ts:28,40`
...
- Prefer `inactive` over `disabled` when the button should "look disabled" but stay reachable by keyboard. `dist/Button/types.d.ts:28,40`
