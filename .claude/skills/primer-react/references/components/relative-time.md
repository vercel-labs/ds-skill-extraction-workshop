...
- `datetime?: string` — the time value as an ISO 8601 string; the standard serializable form. The wrapper parses it (`new Date(datetime)`) and forwards the result as the element's **`date`** property — it does not set the element's own `datetime`. `dist/RelativeTime/RelativeTime.d.ts:4` (conversion `dist/RelativeTime/RelativeTime.js:40-50`; forwarded as `date` at `:64-69`)
