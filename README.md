...
1. **Setup** — clone, install, confirm the skill is on disk, then launch Claude Code.
...
> [!NOTE]
> **Claude Code is the default — not the only path, but the one this README is written against.** Skills live in `.claude/skills/`, which Claude Code auto-loads at startup. Other agentic clients can read the same files, but not all auto-load skills (Cursor, for example, doesn't), so the live follow-along assumes Claude Code.
...
> [!WARNING]
> **Common foot-gun: skill before client.** Don't launch Claude before the skill files are on disk. Claude Code scans `.claude/skills/` at startup — start it in a repo without the skill and `/primer-react` won't exist. Finish steps 1–4 before step 5. Edited the skill after launching? Restart the client so it re-scans.
